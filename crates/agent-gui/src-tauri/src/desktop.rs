//! Desktop-only runtime (Tauri). Only compiled with the `desktop` feature
//! (the default). Everything here depends on Tauri types; the headless build
//! (`--no-default-features`) skips this module entirely.

use std::sync::atomic::{AtomicBool, Ordering};
use std::sync::Arc;

use crate::{commands, runtime, services};
...
fn configure_system_tray(app: &tauri::App) -> tauri::Result<()> {
    let skeleton = services::tray::build_tray_menu_skeleton(app, crate::app_version())?;
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .setup({
            move |app| {
                ...
                let ctx = crate::app_context::AppContext::new(event_emitter);
        ...
        .on_window_event(|window, event| {
            ...
            if let WindowEvent::CloseRequested { api, .. } = event {
                let Some(ctx) = window.try_state::<Arc<crate::app_context::AppContext>>() else {
                    return;
                };
                api.prevent_close();
    ...
    app.run(move |app, event| match event {
        tauri::RunEvent::Resumed => {
            if let Some(ctx) = app.try_state::<Arc<crate::app_context::AppContext>>() {
        ...
        tauri::RunEvent::ExitRequested { api, .. } => {
            let Some(ctx) = app.try_state::<Arc<crate::app_context::AppContext>>() else {
                api.prevent_exit();
                return;
            };
...
fn manage_app_context_states(app: &tauri::App, ctx: &Arc<crate::app_context::AppContext>) {
